...
function miInformacion() {
  ...
  const nombre = "Leonel Zapata Angulo";
  const edad = 19;
  const carrera = "Desarrolo de software y negocios digitales";
...
function tablaMultiplicar(numero) {
  ...
  for (let i = 1; i <= 10; i++) {
    tabla.push(numero * i); // agregamos cada resultado al array
  }
...
function encontrarMaximo(numeros) {
  ...
  let maximo = numeros[0]; // empezamos con el primer elemento

  for (let i = 1; i < numeros.length; i++) {
    if (numeros[i] > maximo) {
      maximo = numeros[i]; // actualizamos si encontramos uno mayor
    }
  }
...
function filtrarMayores(numeros, limite) {
  ...
  for (let i = 0; i < numeros.length; i++) {
    if (numeros[i] > limite) { // comprobamos si es mayor que el límite
      mayores.push(numeros[i]); // lo agregamos al nuevo array
    }
  }
...
function obtenerColumna(matriz, indiceColumna) {
  ...
  for (let i = 0; i < matriz.length; i++) {
    columna.push(matriz[i][indiceColumna]); // tomamos el valor de esa columna en cada fila
  }
...
function transponer(matriz) {
  ...
  for (let i = 0; i < matriz[0].length; i++) { // recorre las columnas originales
    const fila = [];
    for (let j = 0; j < matriz.length; j++) {  // recorre las filas originales
      fila.push(matriz[j][i]); // intercambia fila ↔ columna
    }
